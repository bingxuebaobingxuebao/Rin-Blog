        ...
        <footer>
            <div className="flex flex-col mb-8 space-y-2 justify-center items-center h-16 t-primary">
                ...
                <div className="w-fit-content inline-flex rounded-full border border-zinc-200 p-[3px] dark:border-zinc-700">
                    <ThemeButton mode='light' current={modeState} label={t("theme.light")} icon="ri-sun-line" onClick={setMode} />
                    <ThemeButton mode='system' current={modeState} label={t("theme.system")} icon="ri-computer-line" onClick={setMode} />
                    <ThemeButton mode='dark' current={modeState} label={t("theme.dark")} icon="ri-moon-line" onClick={setMode} />
                </div>
            </div>
        </footer>
    );
...
function ThemeButton({ current, mode, label, icon, onClick }: { current: ThemeMode, label: string, mode: ThemeMode, icon: string, onClick: (mode: ThemeMode) => void }) {
    return (<button aria-label={label} title={label} type="button" onClick={() => onClick(mode)}
