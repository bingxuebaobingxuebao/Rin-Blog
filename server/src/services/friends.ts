...
const friendUpdateBody = t.Object({
    name: t.String(),
    desc: t.String(),
...
export const FriendService = (db: DB, env: Env) => {
    const getFriendList = async ({ admin, uid }: any) => {
    ...
    const addFriend = async ({ admin, uid, set, body: { name, desc, avatar, url } }: any) => {
