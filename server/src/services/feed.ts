...
// Edens Treaty serialises a group root route (e.g. `.group('/feed')` + `.get('/')`)
// to the path `/feed/index`, so every root handler is also registered on `/index`
// to keep older clients working.
...
export const FeedService = (db: DB, env: Env) => {
    const listFeed = async ({ admin, set, query: { page, limit, type } }: any) => {
    ...
    const createFeed = async ({ admin, set, uid, body: { title, alias, listed, content, summary, draft, tags } }: any) => {
        if (!admin) {
            set.status = 403;
            return 'Permission denied';
        }
